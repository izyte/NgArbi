...
export class TableBase extends AppCommonMethods{
  ...
  constructor(public http:HttpClient,public apiUrl:string, public tables?: Array<any> ,public parentTable?:any){ 
    super();
    if(parentTable==undefined) parentTable=null;
  ...
  protected TableLinks():Array<string>{return null;}
  protected Links():Array<any>{return null;}
  ...
  public ChildRowWithKey(childTableCode:string, row:any, groupId?:number):KeyValuePair{
...
*/
  ...
  GetColumnIndices(cols:Array<ColumnInfo>):string{
    ...
    cols.forEach((c:ColumnInfo)=>{
      ret += (ret.length!=0 ? "`" : "") + this.columns.findIndex(e => {return e.name==c.name})
    });
    return ret;
  }
  ...
  Get(args?:{
    ...
    hdrs.set('Access-Control-Allow-Origin', 'Origin, X-Requested-With, Content-Type, Accept');
      ...
      if(args.keyFields!=undefined){
        if(typeof(args.keyFields)=="object"){
          ...
          args.keyFields.forEach(kf => {
            ...
            if(isNaN(kf)){
              tempKeyIndex=String(this.columns.findIndex(c=>{return c.name==kf}));
            }else{
              tempKeyIndex=String(kf);
            }
            prmKeyFields+=(prmKeyFields.length==0 ? "" : "`") + tempKeyIndex;
          });
        }else{
          ...
          if(isNaN(args.keyFields)){
            ...
            prmKeyFields = String(this.columns.findIndex(c=>{return c.name==args.keyFields}));
          }else{
    ...
    let url:string = this.tblUrl + (prmKey.length!=0 ? 
        "/"+prmKey + (prmKeyFields.length!=0 ? 
            "/"+ prmKeyFields : "") : "");
      ...
      (data:any)=>{
        ...
        if(data.recordsList) {
          ...
          retTables.forEach(t=>{
            ...
            if(tbl) 
              tbl.ProcessRequestedRecords(t);
            else
              console.log("Table object '" + t.returnCode + "' not found!");
  ...
  GetRowsByGroup(key?:number,onSuccess?:Function,onError?:Function):Array<any>
  {
    ...
    if(key==undefined){
      ...
      let parKey = this.parentTable.keyCol.name;
      key = this.parentTable.currentRow[parKey];
    }

    let groupKey:string ="g" + key;
    ...
    if(ret!=null){
      if(ret.length!=0) if(!(groupKey in this._GroupRows))this._GroupRows[groupKey] =  ret;
    }
    return ret;
  ...
  UnSubscribe(e:any, abandoned?:boolean){
    ...
    if(abandoned){
      ...
      for(var key in this._TblSubs){
        ...
        console.log("Unsubscribe abandoned!", abandoned, Date.now(), this._UnSubscribeCounter);
        if(subs){
    ...
    let retSubsKey:string = (typeof(e.length)!=undefined ? e[0].subsKey : e.subsKey);
  ...
  GetNewRows():Array<any>{
    return this.GetRows()
    .filter(r=>{return (r._newId != null && r._newId != undefined)})
    .sort((a,b) =>{
